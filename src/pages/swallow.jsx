...
export const Swallow = () => {
    ...
    useEffect(() => {
        async function getRestaurant() {
            try {
                const { data } = await axios.get(`${restaurant}/get/getswallow`);
                dispatch({ type: 'SET_SWALLOW', payload: data });
            } catch (error) {
                console.log(error);
            } finally {
                setLoading(false);
    ...
    const selectedRestaurant = state.swallow && state.swallow.find(data => data._id === id);
