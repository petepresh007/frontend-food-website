...
export const Rice = () => {
    ...
    useEffect(() => {
        async function getRestaurant() {
            try {
                const { data } = await axios.get(`${restaurant}/get/getrice`);
                dispatch({ type: 'SET_RICE', payload: data });
            } catch (error) {
                console.log(error);
            } finally {
                setLoading(false);
    ...
    const selectedRestaurant = state.rice && state.rice.find(data => data._id === id);
