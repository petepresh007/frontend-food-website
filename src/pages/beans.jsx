...
export const Beans = () => {
    ...
    useEffect(() => {
        async function getRestaurant() {
            try {
                const { data } = await axios.get(`${restaurant}/get/getbeans`);
                dispatch({ type: 'SET_BEANS', payload: data });
            } catch (error) {
                console.log(error);
            } finally {
                setLoading(false);
    ...
    const selectedRestaurant = state.beans && state.beans.find(data => data._id === id);


    if (selectedRestaurant.menu.length <= 0) {
        return <div>
            No menu item found, check again later
        </div>
    }
